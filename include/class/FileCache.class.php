...
class FileCache
{
    ...
    public function __construct($path = '', $max_path = 100, $max_file = 50000, $gc_probality = 20)
    {
        $this->path = $path ? $path : HUONIAOROOT . "/data/cache";
    ...
    {
        if (rand(0, 100) < $this->gc_probality) $this->gc();
    ...
    public function gc($path = null)
    {
        ...
        while(($file = readdir($handle)) !== false)
        {
            ...
            {
                //不删除缓存，发现客户网站目录被清空，是这里导致的。 by gz 20190802
                // @unlink($fullPath);
            }
        }
        closedir($handle);
