...
$request->setTargetValue("2"); //根据Target来设定，如Target=device, 则对应的值为 设备id1,设备id2. 多个值使用逗号分隔.(帐号与设备有一次最多100个的限制)
$request->setDeviceType("ANDROID"); //设备类型 ANDROID iOS ALL.
$request->setPushType("NOTICE"); //消息类型 MESSAGE NOTICE
$request->setTitle("php title46"); // 消息的标题
$request->setBody("php body46"); // 消息的内容


// 推送配置: iOS
// $request->setiOSBadge(0); // iOS应用图标右上角角标
$request->setiOSSilentNotification("false");//是否开启静默通知
$request->setiOSMusic("newpaotuifenpeiorder.m4a"); // iOS通知声音
...
// $request->setAndroidOpenUrl("http://www.xxx.com/?service=waimai&do=courier&state=4,5");//Android收到推送后打开对应的url,仅当AndroidOpenType="URL"有效
// $request->setAndroidActivity("com.alibaba.push2.demo.XiaoMiPushActivity");//设定通知打开的activity，仅当AndroidOpenType="Activity"有效
$request->setAndroidMusic("notice");//Android通知音乐
...
$request->setAndroidExtParameters("{\"music\": \"notice\", \"url\":\"http://www.xxx.com/?service=waimai&do=courier&state=4,5\"}"); // 设定android类型设备通知的扩展属性
