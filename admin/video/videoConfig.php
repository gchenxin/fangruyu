 ...
 * 视频频道配置
 *
 * @version        $Id: videoConfig.php 2013-7-7 上午10:33:36 $
...
$templates = "videoConfig.html";

if ($action == "") {
    $action = "video";
}

checkPurview("videoConfig" . $action);
...
if ($type != "") {
    ...
    if ($type == "site") {
		...
		$startlitpic = $_POST['startlitpic'];
		$pauselitpic = $_POST['pauselitpic'];
		$endlitpic   = $_POST['endlitpic'];
		$startTime   = $_POST['startTime'];
		$startUrl    = $_POST['startUrl'];
		$pauseUrl    = $_POST['pauseUrl'];
		$endUrl		 = $_POST['endUrl'];
		$AK  		 = $_POST['AK'];

		$mstartlitpic = $_POST['mstartlitpic'];
		$mpauselitpic = $_POST['mpauselitpic'];
		$mendlitpic   = $_POST['mendlitpic'];
		$mstartTime   = $_POST['mstartTime'];
		$mstartUrl    = $_POST['mstartUrl'];
		$mpauseUrl    = $_POST['mpauseUrl'];
		$mendUrl	  = $_POST['mendUrl'];

		if(empty($AK)){
			die('{"state": 200, "info": ' . json_encode("请填写AK！") . '}');
		}

		$startUrl    = !empty($startUrl) ? $startUrl : '';
		$startTime   = !empty($startTime) ? $startTime : 0;
		$startlitpic = !empty($startlitpic) ? $startlitpic : '';
		$pauseUrl    = !empty($pauseUrl) ? $pauseUrl : '';
		$pauselitpic = !empty($pauselitpic) ? $pauselitpic : '';
		$endUrl      = !empty($endUrl) ? $endUrl : '';
		$endlitpic   = !empty($endlitpic) ? $endlitpic : '';

		$mstartUrl    = !empty($mstartUrl) ? $mstartUrl : '';
		$mstartTime   = !empty($mstartTime) ? $mstartTime : 0;
		$mstartlitpic = !empty($mstartlitpic) ? $mstartlitpic : '';
		$mpauseUrl    = !empty($mpauseUrl) ? $mpauseUrl : '';
		$mpauselitpic = !empty($mpauselitpic) ? $mpauselitpic : '';
		$mendUrl      = !empty($mendUrl) ? $mendUrl : '';
		$mendlitpic   = !empty($mendlitpic) ? $mendlitpic : '';
    ...
    //百度视频
    $customInc .= "\$AK = '" . $AK . "';\r\n";
    $customInc .= "\$startUrl = '" . $startUrl . "';\r\n";
    $customInc .= "\$startTime = " . (int)$startTime . ";\r\n";
    $customInc .= "\$startlitpic = '" . $startlitpic . "';\r\n";
    $customInc .= "\$pauseUrl = '" . $pauseUrl . "';\r\n";
    $customInc .= "\$pauselitpic = '" . $pauselitpic . "';\r\n";
    $customInc .= "\$endUrl = '" . $endUrl . "';\r\n";
    $customInc .= "\$endlitpic = '" . $endlitpic . "';\r\n";
    $customInc .= "\$mstartUrl = '" . $mstartUrl . "';\r\n";
    $customInc .= "\$mstartTime = " . (int)$mstartTime . ";\r\n";
    $customInc .= "\$mstartlitpic = '" . $mstartlitpic . "';\r\n";
    $customInc .= "\$mpauseUrl = '" . $mpauseUrl . "';\r\n";
    $customInc .= "\$mpauselitpic = '" . $mpauselitpic . "';\r\n";
    $customInc .= "\$mendUrl = '" . $mendUrl . "';\r\n";
    $customInc .= "\$mendlitpic = '" . $mendlitpic . "';\r\n";
