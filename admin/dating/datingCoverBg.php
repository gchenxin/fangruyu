 ...
 * 会员中心封面
 *
 * @version        $Id: datingCoverBg.php 2015-7-18 上午1:03:21 $
...
if($dopost != ""){
	$templates = "datingCoverBgAdd.html";

	//js
	$jsFile = array(
		'ui/jquery.dragsort-0.5.1.min.js',
		'publicUpload.js',
		'admin/dating/datingCoverBgAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}else{
	$templates = "datingCoverBg.html";

	//js
	$jsFile = array(
		'ui/bootstrap.min.js',
		'ui/jquery-ui-selectable.js',
		'admin/dating/datingCoverBg.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `title`, `litpic`, `date` FROM `#@__".$tab."` WHERE 1 = 1".$where);
	...
	if(count($results) > 0){
		...
		if(count($list) > 0){
			echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.'}, "datingCoverBg": '.json_encode($list).'}';
		}else{
...
//新增
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`title`, `litpic`, `date`) VALUES ('$title', '$litpic', '$pubdate')");
		$aid = $dsql->dsqlOper($archives, "lastid");

		if($aid){
			adminLog("添加交友会员背景图片", $title);
...
//修改
	...
	checkPurview("datingCoverBgEdit");
	...
	if($submit == "提交"){
		...
		//保存到主表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `litpic` = '$litpic' WHERE `id` = ".$id);
		$return = $dsql->dsqlOper($archives, "update");

		if($return == "ok"){
			adminLog("修改交友会员背景图片", $title);
...
//删除
	...
	if(!testPurview("datingCoverBgDel")){
	...
	if($id != ""){
		...
		if(!empty($error)){
			...
			adminLog("删除交友会员背景图片", join(", ", $title));
