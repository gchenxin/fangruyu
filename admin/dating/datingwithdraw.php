...
if($dopost == "getList"){
	...
	if(!empty($sKeyword)){
		...
		$where1 = array();
		$where1[] = " `order_id` like '%$sKeyword%' OR `cardname` like '%$sKeyword%'";
	...
	if($end != ""){
		$where .= " AND `paydate` <= ". GetMkTime($end." 23:59:59");
	}
	...
	$archives = $dsql->SetQuery("SELECT `id`, `userid`, `bank`, `order_id`, `cardname`, `amount`, `pubdate`, `state` FROM `#@__".$action."` WHERE 1 = 1".$where);
	...
	if(count($results) > 0){
		foreach ($results as $key=>$value) {
			...
			$list[$key]["bank"] = $value["bank"];
			$list[$key]["cardnum"] = $value["order_id"];
			$list[$key]["cardname"] = $value["cardname"];
			$list[$key]["amount"] = $value["amount"];
			$list[$key]["tdate"] = date('Y-m-d H:i:s', $value["pubdate"]);
