 ...
 * 管理投票举报信息
 *
 * @version        $Id: voteComplain.php 2015-6-13 下午21:47:11 $
...
checkPurview($action."Complain");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/".$module;
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = $module."Complain.html";
...
if($dopost == "getDetail"){
	...
	if(count($results) > 0){
		...
		$userSql = $dsql->SetQuery("SELECT `name` FROM `#@__vote_user` WHERE `id` = ". $results[0]['aid']);
		// $typename = $dsql->getTypeName($typeSql);
		$userName = $dsql->dsqlOper($userSql, "results");
		$return["title"] = $userName[0]['name'];
...
//更新举报信息
	...
	$archives = $dsql->SetQuery("UPDATE `#@__".$module."` SET `state` = '$state', `note` = '$note' WHERE `id` = ".$id);
...
//更新举报状态
	...
	$error = array();
	foreach($each as $val){
		$archives = $dsql->SetQuery("UPDATE `#@__".$module."` SET `state` = $arcrank WHERE `id` = ".$val);
		$results = $dsql->dsqlOper($archives, "update");
		if($results != "ok"){
			$error[] = $val;
		}
	...
	if(!empty($error)){
		...
		adminLog("更新分类举报信息状态", $id."=>".$arcrank);
...
//删除举报
	...
	$error = array();
	foreach($each as $val){
		$archives = $dsql->SetQuery("DELETE FROM `#@__".$module."` WHERE `id` = ".$val);
		$results = $dsql->dsqlOper($archives, "update");
		if($results != "ok"){
			$error[] = $val;
		}
...
//获取举报列表
	...
	if($sKeyword != ""){
		...
		//按信息标题搜索
			...
			$archives = $dsql->SetQuery("SELECT `id` FROM `#@__".$module."_user` WHERE `name` like '%$sKeyword%'");
			$results = $dsql->dsqlOper($archives, "results");
			if(count($results) > 0){
				$list = array();
				...
				$idList = join(",", $list);
				$where .= " AND `module` = 'vote' AND `aid` in ($idList)";
			}else{
	...
	$archives = $dsql->SetQuery("SELECT `id` FROM `#@__".$action."` WHERE `module` = '".$module."'");
	...
	$archives = $dsql->SetQuery("SELECT `id`, `aid`, `type`, `userid`, `pubdate`, `ip`, `ipaddr`, `state` FROM `#@__".$action."` WHERE `module` = '".$module."'".$where);
	$results = $dsql->dsqlOper($archives, "results");
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			$typeSql = $dsql->SetQuery("SELECT `name` FROM `#@__".$module."_user` WHERE `id` = ". $value["aid"]);
			$typename = $dsql->getTypeName($typeSql);
			$list[$key]["title"] = $typename[0]['name'];
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/'.$module.'/'.$module.'Complain.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
	$huoniaoTag->compile_dir = HUONIAOROOT."/templates_c/admin/".$module;  //设置编译目录
