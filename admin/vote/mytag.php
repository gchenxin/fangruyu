 ...
 * 模板标记
 *
 * @version        $Id: mytag.php 2014-5-6 下午21:57:18 $
...
if($submit == "预览"){
	...
	if(!empty($valid))   $data["valid"]   = $valid;
	if(!empty($orderby)) $data["orderby"] = $orderby;

	$handels = new handlers($action, "ilist");
...
//新增标记
	...
	if($submit == "提交"){
		...
		$archives = $dsql->SetQuery("INSERT INTO `#@__mytag` (`name`, `module`, `type`, `start`, `end`, `config`, `expbody`, `state`, `pubdate`) VALUES ('$name', '$action', 'ilist', '$start', '$end', '$data', '$expbody', '$state', '".GetMkTime(time())."')");
...
if(file_exists($tpl."/".$templates)){
	...
	//排序
	$huoniaoTag->assign('orderbyList', array(0 => '默认排序', 1 => '发布时间', 2 => '浏览量', 3 => '随机'));
