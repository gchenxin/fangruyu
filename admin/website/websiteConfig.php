 ...
 * 自助建站频道配置
 *
 * @version        $Id: websiteConfig.php 2014-6-12 上午9:21:22 $
...
checkPurview("websiteConfig");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__) . "/../templates/website";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "websiteConfig.html";

$action = $action == "" ? "website" : $action;
...
if ($type != "") {
    ...
    $customInc .= "\$customSiteCheck = '" . $customSiteCheck . "';\r\n";
    //模板风格
...
if (file_exists($tpl . "/" . $templates)) {
    ...
    $jsFile = array(
        ...
        'publicUpload.js',
        'admin/website/websiteConfig.js'
    );
    ...
    $huoniaoTag->assign('siteCheck', $customSiteCheck);

    //站点审核-单选
    $huoniaoTag->assign('siteCheck', array('0', '1'));
    $huoniaoTag->assign('siteCheckNames', array('需要审核', '不需要审核'));
    $huoniaoTag->assign('siteCheckChecked', (int)$customSiteCheck);
