 ...
 * 文章频道配置
 *
 * @version        $Id: articleConfig.php 2013-7-7 上午10:33:36 $
...
$templates = "articleConfig.html";

if($action == ""){
	$action = "article";
}

checkPurview("articleConfig");
...
if($type != ""){
	...
	if($type == "site"){
		...
		$customDelLink         = (int)$delLink;
		$customAutoLitpic      = (int)$autoLitpic;
		$customRewardSwtich    = (int)$rewardSwtich;
	...
	//URL自定义
		...
		$custom_listRule   = (int)$listRule;
		$custom_detailRule = (int)$detailRule;
		adminLog("修改".$title."设置", "URL自定义");

	// 审核流程
	}elseif($type == "audit"){
		$customAuditSwitch = $auditswitch;
		$customAuditAuthChecked = $auditAuth;
		$customAuditType = $auditType;
		adminLog("修改".$title."设置", "审核流程");
	
	}elseif($type == "selfmedia"){
		// $custom_selfmediaSwitch = (int)$selfmediaSwitch;
		// $custom_selfmediaJoinSwitch = (int)$selfmediaJoinSwitch;
		// $custom_selfmediaField = $selfmediaField;
		// $custom_selfmediaLicense = $selfmediaLicense;
		// $custom_selfmediaEditLimit = serialize($selfmediaEditLimit ? $selfmediaEditLimit : array());
		$custom_selfmediaGrantImg = $selfmediaGrantImg;
		$custom_selfmediaGrantTpl = $selfmediaGrantTpl;
		$custom_selfmediaAgreement = $selfmediaAgreement;
	}
	...
	$customInc .= "\$submission = '".$submission."';\r\n";
	$customInc .= "\$customFabuCheck = ".(int)$customFabuCheck.";\r\n";
	$customInc .= "\$customCommentCheck = ".(int)$customCommentCheck.";\r\n";
	$customInc .= "\$customDelLink = ".(int)$customDelLink.";\r\n";
	$customInc .= "\$customAutoLitpic = ".(int)$customAutoLitpic.";\r\n";
	$customInc .= "\$customRewardSwtich = ".(int)$customRewardSwtich.";\r\n";
	//模板风格
	...
	//审核流程
	$customInc .= "\$custom_auditSwitch = ".(int)$customAuditSwitch.";\r\n";
	$customInc .= "\$custom_auditAuth = ".(int)$customAuditAuthChecked.";\r\n";
	$customInc .= "\$custom_auditType = ".(int)$customAuditType.";\r\n";
	// 自媒体
	// $customInc .= "\$custom_selfmediaSwitch = ".$custom_selfmediaSwitch.";\r\n";
	// $customInc .= "\$custom_selfmediaJoinSwitch = ".$custom_selfmediaJoinSwitch.";\r\n";
	// $customInc .= "\$custom_selfmediaField = '".$custom_selfmediaField."';\r\n";
	// $customInc .= "\$custom_selfmediaLicense = '".$custom_selfmediaLicense."';\r\n";
	// $customInc .= "\$custom_selfmediaEditLimit = '".$custom_selfmediaEditLimit."';\r\n";
	$customInc .= "\$custom_selfmediaGrantImg = '".$custom_selfmediaGrantImg."';\r\n";
	$customInc .= "\$custom_selfmediaGrantTpl = '".$custom_selfmediaGrantTpl."';\r\n";
	$customInc .= "\$custom_selfmediaAgreement = '".$custom_selfmediaAgreement."';\r\n";
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'publicUpload.js',
		'admin/article/articleConfig.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));

	require(HUONIAOINC."/config/".$action.".inc.php");
	global $cfg_basehost;
	global $customUpload;
	if($customUpload == 1){
	...
	$huoniaoTag->assign('submission', $submission);
	...
	//删除内容非站内链接-单选
	$huoniaoTag->assign('delLink', array('0', '1'));
	$huoniaoTag->assign('delLinkNames',array('关闭','开启'));
	$huoniaoTag->assign('delLinkChecked', (int)$customDelLink);

	//提取内容首图为缩图-单选
	$huoniaoTag->assign('autoLitpic', array('0', '1'));
	$huoniaoTag->assign('autoLitpicNames',array('关闭','开启'));
	$huoniaoTag->assign('autoLitpicChecked', (int)$customAutoLitpic);

	//文章打赏-单选
	$huoniaoTag->assign('rewardSwitch', array('0', '1'));
	$huoniaoTag->assign('rewardSwitchNames',array('启用','禁用'));
	$huoniaoTag->assign('rewardSwitchChecked', (int)$customRewardSwtich);
	...
	$huoniaoTag->assign('softType_', "*.".str_replace("|", ";*.", $custom_softType));
	$huoniaoTag->assign('thumbSize', $custom_thumbSize);
	...
	$huoniaoTag->assign('listRule', $custom_listRule);
	$huoniaoTag->assign('detailRule', $custom_detailRule);

	//审核开关-单选
	$huoniaoTag->assign('auditswitch', array('0', '1'));
	$huoniaoTag->assign('auditswitchNames',array('关闭','开启'));
	$huoniaoTag->assign('auditswitchChecked', (int)$custom_auditSwitch);

	//权限控制
	$huoniaoTag->assign('auditAuth', array('0', '1', '2', '3'));
	$huoniaoTag->assign('auditAuthNames',array('查看','查看，编辑','查看，删除', '查看，编辑，删除'));
	$huoniaoTag->assign('auditAuthChecked', (int)$custom_auditAuth);

	//审批方式
	$huoniaoTag->assign('auditType', array('0', '1'));
	$huoniaoTag->assign('auditTypeNames',array('主管审批','逐级审批'));
	$huoniaoTag->assign('auditTypeChecked', (int)$custom_auditType);

	// 自媒体--------------
	// $huoniaoTag->assign('selfmediaswitch', array('0', '1'));
	// $huoniaoTag->assign('selfmediaswitchNames',array('关闭','开启'));
	// $huoniaoTag->assign('selfmediaswitchChecked', (int)$custom_selfmediaSwitch);

	// $huoniaoTag->assign('selfmediajoinswitch', array('0', '1'));
	// $huoniaoTag->assign('selfmediajoinswitchNames',array('关闭','开启'));
	// $huoniaoTag->assign('selfmediajoinswitchChecked', (int)$custom_selfmediaJoinSwitch);
	// $huoniaoTag->assign('selfmediaField', $custom_selfmediaField);
	// $huoniaoTag->assign('selfmediaLicense', $custom_selfmediaLicense);
	// $huoniaoTag->assign('selfmediaEditLimit', $custom_selfmediaEditLimit ? unserialize($custom_selfmediaEditLimit) : array());

	$huoniaoTag->assign('selfmediaGrantImg', $custom_selfmediaGrantImg);
	$huoniaoTag->assign('selfmediaGrantTpl', $custom_selfmediaGrantTpl);
	$huoniaoTag->assign('selfmediaAgreement', $custom_selfmediaAgreement);
	$huoniaoTag->assign('selfmediaInit', isset($custom_selfmediaJoinSwitch) ? 0 : 1);
