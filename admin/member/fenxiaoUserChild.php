...
if($action == "getList"){
  ...
  $archives = $dsql->SetQuery("SELECT f.`id`, m.`id` uid, m.`mtype`, m.`username`, m.`nickname`, m.`realname`, m.`from_uid`, m2.`username` recuser, m2.`mtype` from_mtype, f.`state`, f.`pubdate`, f.`phone` FROM `#@__member` m LEFT JOIN `#@__member_fenxiao_user` f ON m.`id` = f.`uid` LEFT JOIN `#@__member` m2  ON m2.`id` = m.`from_uid` WHERE 1 = 1".$where);
  ...
  if($results){
    ...
    foreach ($results as $key => $value) {
      ...
      $list[$key]['phone'] = $value['phone'] != null ? $value['phone'] : '';
