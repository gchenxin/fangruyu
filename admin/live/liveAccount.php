...
if($action != "" || $dopost != ""){
	...
	//启用直播帐号
		...
		if($dopost == "add"){
			...
			$archives = $dsql->SetQuery("INSERT INTO `#@__liveaccount` (`title`, `username`, `password`, `vhost`, `appname`, `pushdomain`, `privatekey`, `website`, `contact`, `state`) VALUES ('$title', '$username', '$password', '$vhost', '$appname', '$pushdomain', '$privatekey',  '$website', '$contact', '$state')");
			$results = $dsql->dsqlOper($archives, "update");
			...
			if($id !== ""){
				...
				$archives = $dsql->SetQuery("UPDATE `#@__liveaccount` SET `title` = '$title', `username` = '$username', `password` = '$password', `vhost` = '$vhost', `appname` = '$appname', `pushdomain` = '$pushdomain', `privatekey` = '$privatekey', `website` = '$website', `contact` = '$contact' WHERE `id` = $id");
				$results = $dsql->dsqlOper($archives, "update");
