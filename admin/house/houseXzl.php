...
if($dopost == "getList"){
	...
	if($sKeyword != ""){
		...
		if(!empty($sidArr)){
			$where .= " AND (`title` like '%$sKeyword%' OR `loupan` like '%$sKeyword%' OR `contact` like '%$sKeyword%' OR `userid` in (".join(",",$sidArr)."))";
		}else{
			$where .= " AND (`title` like '%$sKeyword%' OR `loupan` like '%$sKeyword%' OR `contact` like '%$sKeyword%')";
		}
	}
...
//删除
	...
	if($id != ""){
		...
		foreach($each as $val){
			...
			//删除举报信息
			$archives = $dsql->SetQuery("DELETE FROM `#@__member_complain` WHERE `module` = 'house' AND `action` = 'xzl' AND `aid` = ".$val);
