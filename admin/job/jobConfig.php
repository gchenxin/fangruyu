 ...
 * 招聘频道配置
 *
 * @version        $Id: jobConfig.php 2014-3-14 上午11:49:02 $
 * @package        HuoNiao.Job
...
checkPurview("jobConfig");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/job";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "jobConfig.html";
...
if($type != ""){
	...
	if($type == "site"){
		...
		$custom_fair_atlasMax  = (int)$fair_atlasMax;
		$resume_point          = (int)$_POST['resume_point'];
		$custom_fabuCheck      = (int)$fabuCheck;
	...
	$customInc .= "\$custom_fair_atlasMax = ".$custom_fair_atlasMax.";\r\n";
	$customInc .= "\$resume_point = ".(int)$resume_point.";\r\n";
	$customInc .= "\$custom_fabuCheck = ".(int)$custom_fabuCheck.";\r\n";
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('fair_atlasMax', $custom_fair_atlasMax);
	$huoniaoTag->assign('resume_point', (int)$resume_point);

	$huoniaoTag->assign('fabuCheckNum', array('0', '1'));
	$huoniaoTag->assign('fabuCheckNames', array('需要审核', '不需要审核'));
	$huoniaoTag->assign('fabuCheckChecked', (int)$custom_fabuCheck);
