define(function(require) {
    ...
    c = {
        getArticleType: "/include/"+Module+".inc.php?action=articleType&projectid="+g
    },
    f = "app-articleType-display";
    s.createSection(f, {
        initView: function() {
    ...
    F = i({
        setup: function(t) {
            ...
            this.content = $('<ul class="w-articleType-container">loading...</ul>'),
        ...
        loadData: function() {
            ...
            function(e) {
                ...
                if (t.content.empty(), e.data) {
                    var i = new m('<li><a href="article-list-{%catid%}.html">{%catname%}'+(t.getParam("showCount") ? ' ({%count%})' : "")+'</a></li>');
                    $.each(e.data,
                    function(e, n) {
                        var s = $.extend({},
                        n);
            ...
            return $.getJSON(c.getArticleType + "&jsoncallback=?", {
                projectid: g,
